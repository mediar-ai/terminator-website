export async function addToWaitlist(
  ...
  try {
    ...
    if (!contactResponse.ok) {
      ...
      const errorBody = await contactResponse.text().catch(() => "");
      console.error(`Loops contact create failed: ${status}`, errorBody);
      ...
      if (status === 400) {
        ...
        if (!updateResponse.ok) {
          const updateErrorBody = await updateResponse.text().catch(() => "");
          console.error(`Loops contact update failed: ${updateResponse.status}`, updateErrorBody);
    ...
    // Step 2: Send transactional email to user (non-blocking - waitlist signup succeeds even if email fails)
    if (sendEmail && process.env.LOOPS_API_KEY && process.env.LOOPS_TRANSACTIONAL_ID) {
      ...
      try {
        ...
        // Log email error but don't fail the waitlist signup
