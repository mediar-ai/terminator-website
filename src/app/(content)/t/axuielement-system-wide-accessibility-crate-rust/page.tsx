...
const faqs: FaqItem[] = [
  ...
  {
    q: "What exactly do I add to Cargo.toml to get AXUIElement on macOS, and where is it on docs.rs?",
    a: "Add accessibility = \"0.2\" for the high-level wrapper, and core-foundation for the CFArray and TCFType helpers you need to downcast attribute results. The crate page is docs.rs/accessibility/latest/accessibility and the type itself lives in the ui_element module at docs.rs/accessibility/latest/accessibility/ui_element/struct.AXUIElement.html. Be warned that the published docs are nearly empty (the crate shows 0% documented on docs.rs), which is why the signature is all you get and why this guide exists. The macOS-only gate matters too: AXUIElement only compiles and links on macOS, so in a cross-platform crate you put the dependency behind [target.'cfg(target_os = \"macos\")'.dependencies]. Terminator's own tree walk in crates/terminator/src/platforms/tree_search.rs opens with exactly this import: use accessibility::{AXAttribute, AXUIElement, AXUIElementAttributes, Error}; followed by use core_foundation::array::CFArray; and use core_foundation::base::TCFType;. If you want the raw C symbols instead of the wrapper, depend on accessibility-sys and call AXUIElementCreateSystemWide() yourself.",
  },
  {
